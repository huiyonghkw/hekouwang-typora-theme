...
def main():
    ...
    for preset_name in built:
        t, preset = apply_reading_preset(tokens, preset_name)
        suffix = preset.get("slug_suffix", "")
        label = preset.get("label", preset_name)
        for dark in (False, True):
            name = f"{slug}{suffix}{'-dark' if dark else ''}.css"
            banner = (
                f"hekouwang{suffix}{'-dark' if dark else ''} — a Typora theme"
                f" ({label}{', dark' if dark else ''})"
            )
            ...
            problems = check(css)
            tag = f"{preset_name}/{'dark' if dark else 'light'}"
